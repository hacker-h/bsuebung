...
RUN_COMMANDS="cp -r /MyFS/* ${CONTAINER_DIR}; cd ${CONTAINER_DIR}; ls obj; make; bash" #commands to run after starting the container additional to the ssh daemon ('bash' starts an interactive shell)
...
Password: root
"
docker run -it --rm -v "/${HOST_DIR}:/${CONTAINER_DIR}" -v "obj:/MyFS/obj" --name "${CONTAINER_NAME}" -p "${SSH_PORT}":22 "${IMAGE_NAME}" bash -c "/usr/sbin/sshd; ${RUN_COMMANDS}"
